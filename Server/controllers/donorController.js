...
const handleLogin = async (req, res) => {
  ...
  try {
    const foundUser = await donor.findOne({ email: email }).exec();
...
const getAllDonors = async (req, res) => {
  try {
    const allData = await donor.find();
...
const getDonor = async (req, res) => {
  ...
  try {
    const user = await donor.findById(userId).exec();
